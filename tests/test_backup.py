...
def _make_mocks(text="/backup", user_id=12345678, owner_id=12345678):
    ...
    config.get_all_api_keys.return_value = {
        "OPENAI_API_KEY": "sk-test",
        "MISTRAL_API_KEY": "xm-test",
    }
...
async def test_backup_sends_document():
    ...
    with patch("nanoclaw_bot.agents.AgentManager") as mock_mgr:
    ...
    assert "nanoclaw_backup_" in call_kwargs.kwargs.get(
        "filename",
        call_kwargs[1].get("filename", ""),
    )
...
async def test_backup_no_secrets():
    ...
    with (
        patch("nanoclaw_bot.agents.AgentManager") as mock_mgr,
